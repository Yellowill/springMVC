...
public class IDUtil {
     ...
     * 2、地址码(前六位数）表示编码对象常住户口所在县(市、旗、区)的行政区划代码，按GB/T2260的规定执行。 6
     * 3、出生日期码（第七位至十四位） 表示编码对象出生的年、月、日，按GB/T7408的规定执行，年、月、日代码之间不用分隔符。8
     * 4、顺序码（第十五位至十七位） 表示在同一地址码所标识的区域范围内，对同年、同月、同日出生的人编定的顺序号， 顺序码的奇数分配给男性，偶数分配给女性。3
     * 5、校验码（第十八位数） 1
    ...
    public IDInfo IDCardValidate(String IDStr) {
        ...
        Date brirthday = null;
        try {
            brirthday = DateUtils.parse4yyyyMMdd(strYear + "-" + strMonth + "-" + strDay);
        ...
        if(Integer.parseInt(Ai.substring(16,17))% 2 == 0){
            idInfo.gender = PolicyConst.Gender.FEMALE;
        }else{
            idInfo.gender = PolicyConst.Gender.MALE;
        }
    ...
    public static void main(String[] args) throws ParseException {
        ...
        String IDCardNum = "44011119350915242X";
